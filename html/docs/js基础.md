...
在 JavaScript 中，使用 for...of 循环遍历可迭代对象时是有序的。对于数组和字符串，它们会按照索引顺序进行迭代

对于普通对象（非数组或字符串），for...of 循环无法保证迭代顺序

### for in

在 JavaScript 中，使用 for...in 循环遍历对象的键时是无序的。根据规范，for...in 循环会按照以下顺序遍历对象的属性：

先遍历所有数字键（按照升序排列）。
然后按照插入顺序遍历字符串键。
最后按照插入顺序遍历符号键。
但请注意，虽然按照规范来说，数字键会先于字符串键和符号键被遍历，但实际上，不同 JavaScript 引擎可能对此有所差异，并不能保证完全一致。

因此，如果你需要按照特定顺序遍历对象的属性，最好使用其他方法，比如将对象属性存储在数组中并进行排序，或者使用 ES6 中的 Map 或 Set 来保存键值对。
